...
export default function PostFeed({ user }: { user: User }) {
  ...
  // Use a ref to always have access to the latest fetchPosts in the stable subscription effect
  const fetchPostsRef = useRef(fetchPosts)
  useEffect(() => {
    fetchPostsRef.current = fetchPosts
  }, [fetchPosts])

  // Effect for initial and search/filter-based loading
  useEffect(() => {
    void fetchPosts()
  }, [fetchPosts])

  // Stable effect for Realtime subscription
  useEffect(() => {
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-card p-3 rounded-xl shadow-sm border border-border">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-secondary/50 border border-border rounded-lg focus:ring-2 focus:ring-primary text-sm text-foreground outline-none"
            value={search}
