...
export default function PostFeed({ user }: { user: User }) {
  ...
  // Initial fetch on search/filter change
  useEffect(() => {
  ...
  return (
    <div className="space-y-6 pb-20">
      <CreatePost user={user} onPostCreated={() => { if (fetchPostsRef.current) void fetchPostsRef.current() }} />
      ...
      <div className="flex flex-col sm:flex-row gap-3 bg-card p-3 rounded-2xl shadow-sm border border-border">
        <div className="relative flex-1">
          ...
          <input
            ...
            className="w-full pl-10 pr-4 py-2 bg-background border-none rounded-xl focus:ring-2 focus:ring-primary text-sm text-foreground"
            value={search}
      ...
      {loading ? (
        ...
        <div className="space-y-4">
          {posts.map((post) => (
            <PostCard key={post.id} post={post} currentUser={user} onUpdate={() => { if (fetchPostsRef.current) void fetchPostsRef.current() }} />
          ))}
          {posts.length === 0 && (
