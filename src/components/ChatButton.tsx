  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 bg-blue-600 text-white rounded-full shadow-lg flex items-center justify-center hover:bg-blue-700 transition-all z-40 hover:scale-110 active:scale-95"
      >
        ...
        <div className="fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 sm:w-96 h-full sm:h-[550px] bg-card shadow-2xl z-50 flex flex-col sm:rounded-3xl border border-border overflow-hidden transition-all animate-in slide-in-from-right-4">
          ...
          <div ref={scrollRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-background/50">
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.user_id === user.id ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[85%] p-3 rounded-2xl text-sm ${
                  msg.user_id === user.id ? 'bg-blue-600 text-white rounded-tr-none' : 'bg-card border border-border text-foreground rounded-tl-none shadow-sm'
                }`}>
                  {msg.content}
                </div>
              </div>
            ...
            {messages.length === 0 && (
              <div className="text-center py-10">
                <p className="text-muted text-sm">Pas encore de messages. Commencez la discussion !</p>
              </div>
            )}
          </div>
          ...
          <form onSubmit={(e) => void sendMessage(e)} className="p-4 border-t border-border bg-card flex gap-2">
            <input
              ...
              className="flex-1 bg-background border border-border rounded-full px-4 py-2 text-sm focus:ring-2 focus:ring-blue-500 outline-none transition-all"
              value={newMessage}
            ...
            <button
              ...
              className="bg-blue-600 text-white p-2.5 rounded-full hover:bg-blue-700 disabled:opacity-50 transition-all active:scale-95"
            >
              <Send size={18} />
            </button>
          </form>
