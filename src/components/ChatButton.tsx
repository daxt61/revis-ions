...
import { ChatMessage, Profile } from '@/utils/types'
...
export default function ChatButton({ user }: { user: User }) {
    ...
    if (isOpen) {
      ...
      const channel = supabase
        ...
        .on('postgres_changes' as any, { event: 'INSERT', table: 'chat_messages' }, async (payload: any) => {
          // Optimized Realtime: Fetch only the specific profile related to the new message payload
  ...
  return (
    <>
      {/* Floating Button - Right Positioned */}
      <button
      ...
      {isOpen && (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 sm:w-96 h-full sm:h-[500px] bg-card shadow-2xl z-50 flex flex-col sm:rounded-2xl border border-border overflow-hidden transition-all animate-in slide-in-from-right-4">
          <div className="bg-primary p-4 text-white flex items-center justify-between">
            ...
            <button onClick={() => setIsOpen(false)} className="hover:bg-primary/20 p-1 rounded-full">
              <X size={20} />
            </button>
          </div>
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.user_id === user.id ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                  msg.user_id === user.id ? 'bg-primary text-white rounded-tr-none' : 'bg-card border border-border rounded-tl-none shadow-sm'
                }`}>
                  {msg.content}
                </div>
              </div>
          ...
          <form onSubmit={sendMessage} className="p-4 border-t border-border bg-card flex gap-2">
            <input
              ...
              className="flex-1 bg-muted/5 border-none rounded-full px-4 py-2 text-sm text-foreground focus:ring-2 focus:ring-primary placeholder:text-muted"
              value={newMessage}
