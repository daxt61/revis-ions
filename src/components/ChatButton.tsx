...
export default function ChatButton({ user }: { user: User }) {
  ...
  return (
    <>
      ...
      {/* Chat Drawer */}
      {isOpen && (
        <div className="fixed inset-0 sm:inset-auto sm:bottom-24 sm:right-6 sm:w-96 h-full sm:h-[500px] bg-card shadow-2xl z-50 flex flex-col sm:rounded-3xl border border-border overflow-hidden transition-all animate-in slide-in-from-right-4">
            ...
            {messages.map((msg) => (
              <div key={msg.id} className={`flex flex-col ${msg.user_id === user.id ? 'items-end' : 'items-start'}`}>
                ...
                <div className={`max-w-[80%] p-3 rounded-2xl text-sm ${
                  msg.user_id === user.id ? 'bg-primary text-white rounded-tr-none' : 'bg-white/10 text-foreground border border-border rounded-tl-none shadow-sm'
                }`}>
                  {msg.content}
                </div>
              </div>
          ...
          <form onSubmit={sendMessage} className="p-4 border-t border-border bg-card flex gap-2">
            <input
              ...
              className="flex-1 bg-white/5 border-none rounded-full px-4 py-2 text-sm text-foreground focus:ring-2 focus:ring-primary placeholder-muted"
              value={newMessage}
            ...
            <button
              ...
              className="bg-primary text-white p-2 rounded-full hover:bg-primary/90 disabled:opacity-50 transition-all transform active:scale-90"
            >
              <Send size={18} />
            </button>
          </form>
