  ...
  return (
    <div className="bg-card rounded-2xl shadow-sm border border-border p-4 sticky top-20">
      ...
      <ul className="space-y-3">
        {profiles.map((profile) => (
          <li key={profile.id} className="flex items-center gap-3">
            <div className="relative">
              <div className="w-8 h-8 bg-secondary rounded-full flex items-center justify-center text-primary font-bold text-xs border border-border">
                {profile.username?.charAt(0).toUpperCase() || '?'}
              </div>
              <div
                className={`absolute -bottom-0.5 -right-0.5 w-3 h-3 rounded-full border-2 border-card ${
                  onlineUsers.has(profile.id) ? 'bg-green-500 shadow-[0_0_8px_rgba(34,197,94,0.5)]' : 'bg-muted-foreground/30'
                }`}
              />
            </div>
        ...
        {profiles.length === 0 && <p className="text-xs text-muted-foreground">Aucun utilisateur</p>}
      </ul>
    </div>
  )
}
