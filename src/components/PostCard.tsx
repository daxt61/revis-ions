...
export default function PostCard({ post, currentUser, onUpdate }: { post: any, currentUser: any, onUpdate: () => void }) {
  ...
  return (
    <div className={`bg-card rounded-2xl shadow-sm border border-border overflow-hidden flex ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-blue-500'}`}>
      {/* Vote Sidebar */}
      <div className="bg-background/50 w-12 flex flex-col items-center py-2 gap-1 border-r border-border">
        <button
          onClick={() => void handleVote(1)}
          className={`p-1 rounded transition-colors ${userVote === 1 ? 'text-orange-500 bg-orange-500/10' : 'text-muted hover:bg-border'}`}
        >
          <ArrowBigUp size={24} fill={userVote === 1 ? 'currentColor' : 'none'} />
        </button>
        <span className={`text-sm font-bold ${score > 0 ? 'text-orange-500' : score < 0 ? 'text-blue-500' : 'text-foreground'}`}>
          {score}
        </span>
        <button
          onClick={() => void handleVote(-1)}
          className={`p-1 rounded transition-colors ${userVote === -1 ? 'text-blue-500 bg-blue-500/10' : 'text-muted hover:bg-border'}`}
        >
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-border text-foreground px-2 py-1 rounded-md">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-border mt-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors text-sm font-medium ${
              showComments ? 'text-blue-500 bg-blue-500/10' : 'text-muted hover:bg-border'
            }`}
          >
