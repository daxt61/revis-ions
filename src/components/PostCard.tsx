...
export default function PostCard({ post, currentUser, onUpdate }: { post: any, currentUser: any, onUpdate: () => void }) {
  ...
  return (
    <div className={`bg-card rounded-2xl shadow-sm border overflow-hidden flex transition-colors ${
      ...
      {/* Vote Sidebar */}
      <div className="bg-muted/5 w-12 flex flex-col items-center py-2 gap-1 border-r border-border">
        <button
      ...
      <div className="flex-1 p-4">
        <div className="flex items-center gap-2 mb-2">
          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded-full ${
            isNoublionsPas ? 'bg-orange-500/20 text-orange-600' : 'bg-primary/20 text-primary'
          }`}>
            {post.type === "n'oublions pas" ? "N'oublions pas" : "Révis-ions"}
          </span>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-muted/10 text-muted px-2.5 py-1 rounded-full border border-border">
              <BookOpen size={14} />
              {post.subject}
            </div>
          )}
          {post.due_date && (
            <div className="flex items-center gap-1.5 text-xs bg-red-500/10 text-red-500 px-2.5 py-1 rounded-full font-medium border border-red-500/20">
              <Calendar size={14} />
