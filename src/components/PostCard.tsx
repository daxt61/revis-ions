...
export default function PostCard({ post, currentUser, onUpdate }: { post: any, currentUser: User, onUpdate: () => void }) {
  ...
  return (
    <div className={`bg-card rounded-xl shadow-sm border border-border overflow-hidden flex ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-primary'}`}>
      {/* Vote Sidebar */}
      <div className="bg-secondary/30 w-12 flex flex-col items-center py-2 gap-1 border-r border-border">
        <button
          onClick={() => handleVote(1)}
          className={`p-1 rounded transition-colors ${userVote === 1 ? 'text-orange-500 bg-orange-500/10' : 'text-muted hover:bg-secondary'}`}
        >
        ...
        <button
          onClick={() => handleVote(-1)}
          className={`p-1 rounded transition-colors ${userVote === -1 ? 'text-primary bg-primary/10' : 'text-muted hover:bg-secondary'}`}
        >
      ...
      <div className="flex-1 p-4">
        <div className="flex items-center gap-2 mb-2">
          <span className={`text-[10px] font-bold uppercase px-2 py-0.5 rounded ${
            isNoublionsPas ? 'bg-orange-500/20 text-orange-500' : 'bg-primary/20 text-primary'
          }`}>
            {post.type}
          </span>
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-secondary text-foreground px-2 py-1 rounded-md border border-border">
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-border mt-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors text-sm font-medium ${
              showComments ? 'text-primary bg-primary/10' : 'text-muted hover:bg-secondary'
            }`}
          >
