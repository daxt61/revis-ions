...
export default function PostCard({ post, currentUser, onUpdate }: { post: Post, currentUser: { id: string }, onUpdate: () => void }) {
  ...
  return (
    <div className={`bg-card rounded-2xl shadow-md border border-border overflow-hidden flex transition-all hover:shadow-lg ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-primary'}`}>
      {/* Vote Sidebar */}
      <div className="bg-muted/20 w-12 flex flex-col items-center py-2 gap-1 border-r border-border">
        <button
          onClick={() => handleVote(1)}
          className={`p-1 rounded transition-colors ${userVote === 1 ? 'text-orange-500 bg-orange-500/10' : 'text-muted-foreground hover:bg-muted'}`}
        >
        ...
        <button
          onClick={() => handleVote(-1)}
          className={`p-1 rounded transition-colors ${userVote === -1 ? 'text-primary bg-primary/10' : 'text-muted-foreground hover:bg-muted'}`}
        >
      ...
      <div className="flex-1 p-4">
        ...
        {post.description && (
          <p className="text-muted-foreground text-sm mb-4 line-clamp-3 whitespace-pre-wrap">{post.description}</p>
        )}

        <div className="flex flex-wrap gap-3 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-muted text-muted-foreground px-2.5 py-1 rounded-full border border-border">
              <BookOpen size={14} />
              {post.subject}
            </div>
          )}
          {post.due_date && (
            <div className="flex items-center gap-1.5 text-xs bg-destructive/10 text-destructive px-2.5 py-1 rounded-full border border-destructive/20 font-medium">
              <Calendar size={14} />
