    ...
    <div className={`bg-card rounded-2xl shadow-sm border border-border overflow-hidden flex transition-all hover:border-muted/50 ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-primary'}`}>
      ...
      <div className="bg-background/50 w-12 flex flex-col items-center py-4 gap-1 border-r border-border">
        <button
          ...
          className={`p-1 rounded-lg transition-colors ${userVote === 1 ? 'text-orange-500 bg-orange-500/10' : 'text-muted hover:bg-muted/10'}`}
        >
        ...
        <button
          onClick={() => { void handleVote(-1) }}
          className={`p-1 rounded-lg transition-colors ${userVote === -1 ? 'text-primary bg-primary/10' : 'text-muted hover:bg-muted/10'}`}
        >
      ...
      <div className="flex-1 p-4 sm:p-6">
        ...
        <div className="flex flex-wrap gap-3 mb-6">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-background border border-border text-muted px-2.5 py-1 rounded-lg">
              <BookOpen size={14} className="text-primary" />
              {post.subject}
            </div>
          )}
          {post.due_date && (
            <div className="flex items-center gap-1.5 text-xs bg-red-500/10 text-red-500 px-2.5 py-1 rounded-lg font-medium border border-red-500/20">
              <Calendar size={14} />
