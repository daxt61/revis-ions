...
export default function PostCard({ post, currentUser, onUpdate }: { post: Post, currentUser: User, onUpdate: () => void }) {
  ...
  const votes = (post.votes as any[]) || []
  const userVote = votes.find((v: any) => v.user_id === currentUser.id)?.value || 0
  const score = votes.reduce((acc: number, v: any) => acc + v.value, 0) || 0
  ...
  const commentCount = Array.isArray(post.comments) ? (post.comments[0] as any)?.count || 0 : (post.comments || 0)

  return (
    <div className={`bg-card rounded-2xl shadow-sm border border-border overflow-hidden flex transition-all hover:shadow-md ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-primary'}`}>
      {/* Vote Sidebar */}
      <div className="bg-background/50 w-12 flex flex-col items-center py-4 gap-2 border-r border-border">
        <button
      ...
      <div className="flex-1 p-5">
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-muted text-muted-foreground px-2.5 py-1 rounded-lg border border-border">
              <BookOpen size={14} />
              {post.subject}
            </div>
          )}
          {post.due_date && (
            <div className="flex items-center gap-1.5 text-xs bg-destructive/10 text-destructive px-2.5 py-1 rounded-lg border border-destructive/20 font-medium">
              <Calendar size={14} />
          ...
          <div className={`grid gap-3 mb-4 ${post.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-3'}`}>
            {post.images.map((img: string, i: number) => (
              <img
                ...
                src={img}
                alt="post image"
                className="w-full aspect-video object-cover rounded-xl border border-border shadow-sm cursor-pointer hover:opacity-90 transition-all hover:scale-[1.02]"
