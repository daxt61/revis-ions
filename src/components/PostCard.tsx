...
export default function PostCard({ post, currentUser, onUpdate }: PostProps) {
  ...
  return (
    <div className={`bg-card rounded-xl shadow-sm border border-border overflow-hidden flex ${isNoublionsPas ? 'border-l-4 border-l-orange-500' : 'border-l-4 border-l-primary'}`}>
      {/* Vote Sidebar */}
      <div className="bg-border/30 w-12 flex flex-col items-center py-2 gap-1 border-r border-border">
        <button
          onClick={() => handleVote(1)}
          className={`p-1 rounded transition-colors ${userVote === 1 ? 'text-orange-600 bg-orange-500/10' : 'text-muted hover:bg-border'}`}
        >
          <ArrowBigUp size={24} fill={userVote === 1 ? 'currentColor' : 'none'} />
        </button>
        <span className={`text-sm font-bold ${score > 0 ? 'text-orange-600' : score < 0 ? 'text-primary' : 'text-foreground'}`}>
          {score}
        </span>
        <button
          onClick={() => handleVote(-1)}
          className={`p-1 rounded transition-colors ${userVote === -1 ? 'text-primary bg-primary/10' : 'text-muted hover:bg-border'}`}
        >
      ...
      <div className="flex-1 p-4">
        ...
        <div className="flex flex-wrap gap-3 mb-4">
          {post.subject && (
            <div className="flex items-center gap-1.5 text-xs bg-border text-muted px-2 py-1 rounded-md">
              <BookOpen size={14} />
              {post.subject}
            </div>
          )}
          {post.due_date && (
            <div className="flex items-center gap-1.5 text-xs bg-red-500/10 text-red-500 px-2 py-1 rounded-md font-medium">
              <Calendar size={14} />
          ...
          <div className={`grid gap-2 mb-4 ${post.images.length === 1 ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-3'}`}>
            {post.images.map((img: string, i: number) => (
              <img
                ...
                src={img}
                alt="post image"
                className="w-full aspect-square object-cover rounded-lg border border-border shadow-sm cursor-pointer hover:opacity-90 transition-opacity"
        ...
        <div className="flex items-center gap-4 pt-2 border-t border-border mt-2">
          <button
            ...
            className={`flex items-center gap-2 px-3 py-1.5 rounded-lg transition-colors text-sm font-medium ${
              showComments ? 'text-primary bg-primary/10' : 'text-muted hover:bg-border'
            }`}
          >
